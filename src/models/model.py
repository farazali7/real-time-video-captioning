...
from image_encoders import TinyVIT
...
class StudentCandidate(CaptioningModel):
    '''
    Work in Progress, I would suggest we come up with a complete model that doesn't require the teacher model using nn.module
    '''
    def __init__(self, image_encoder, text_decoder, decoder, tokenizer, param):
        ...
        self.teacher,self.teacher_param= self.get_teacher_model()
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.teacher.eval()
        y_teacher=perform_inference(self.teacher, '', self.teacher_param, x)
        #Tokenize y???
        #Can do Teacher Forcing?
        #Can do KL Divergence between Teacher Final Logits Student Final Logits
        #Can do MSE Loss with Teacher Final Encoder Output Student Encoder OUtput
    ...
    def get_teacher_model(self,model_path='model.pt',model_name='GIT_LARGE_MSRVTT',yaml_path='GIT_LARGE_MSRVTT_parameter.yaml'):
        def custom_infer(self, batch, visual_features, visual_features_valid, search_param=None):
        ...
        def custom_search(input_ids,step,num_keep_best= 1,do_sample=False,top_k=None,top_p=None,num_return_sequences=1):
        ...
        ckpt = ckpt['model']
        param = load_from_yaml_file(yaml_path)
        model = get_git_model(self.tokenizer, param)
        model.infer = custom_infer.__get__(model, CaptioningModel)
        model.decoder.search = custom_search.__get__(model.decoder, model.decoder.__class__)
        model.eval()
